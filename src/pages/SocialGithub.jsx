...
import SocialGithub from "../components/RenderContent/SocialGithub";

const SocialGithubPage = () => {
  const { addTerminalLine, clearCount } = useTerminal();
  const hasAddedContent = useRef(false);
  const lastClearCount = useRef(0);

  const addGithubContent = useCallback(() => {
    addTerminalLine({
      type: "output",
  ...
  // Add GitHub content to terminal - only once, in useEffect
  useEffect(() => {
  ...
  }, [clearCount, addGithubContent]);

  return <TerminalLayout />;
};

export default SocialGithubPage;
